...
def add_slope_information(divergence: dict[str, Any], candles_df: pd.DataFrame, pivot_window: int) -> dict[str, Any]:
    '''
    ...
    + pivot_window: Número de velas de amplitud usadas para determinar los pivotes.

    Retorno:
    + La divergencia (con información adicional).
    '''
    slope_batch = pivot_window * 2 #Número de velas utilizadas en cada iteración.
...
def add_volume_information(divergence: dict[str, Any], vol_series: pd.Series) -> dict[str, Any]:
    ...
    if latest_ratio == max_ratio: #si el último ratio es el máximo tras superar el bucle, significa que sí.
        divergence['strength'] += 1
...
def check_divergence(divergence: dict[str, Any], min_strength: int) -> bool:
    '''
    Comprueba si la divergencia cumple los criterios de calidad.

    Parámetros:
    + divergence: El diccionario con información de la divergencia.
    + min_strength: La intensidad mínima que se espera de un divergencia.
...
def check_low_pivots(low_pivots_idx: list[int], pivot_limit: int) -> bool:
    '''
    Comprueba si los mínimos cumplen las condiciones de divergencia.

    Parámetros:
    + low_pivots_idx: Una lista con los índices de los mínimos locales.
    + pivot_limit: El índice por debajo del cual no puede estar el último mínimo de divergencia.

    Retorno:
    + Un valor lógico en función de si se cumplen o no las condiciones.
    '''
    filter_1 = len(low_pivots_idx) < 2 #Se requieren al menos dos pivotes
    filter_2 = low_pivots_idx[-1] < pivot_limit #y que el último de ellos esté por encima del límite.

    if filter_1 or filter_2: return False
...
def find_bullish_divergence(divergence_df: pd.DataFrame) -> dict[str, Any]:
    '''
...
def find_low_pivots(low_series: pd.Series, pivot_window: int) -> list[int]:
    '''
    Detecta mínimos locales en la serie de precios mínimos del DF de mercado.

    Parámetros:
    + low_series: La serie de precios mínimos.
    ...
    for i in range(pivot_window, len(low_series) - pivot_window): #Se evalúan sólos los puntos con suficiente contexto a su alrededor.
        ...
        if low_series.iloc[i] == min(low_series.iloc[start:end]): #Calcula el mínimo dentro de la ventana y lo compara con el valor en iteración;
            pivots_idx.append(i) #si coinciden, se trata de un mínimo local, por lo que añade su índice al conjunto.
    
    last_i = len(low_series) - pivot_window + low_series.iloc[-pivot_window:].argmin() #Además, se busca el mínimo del final de la serie, sin preocuparse de la amplitud de la ventana hacia la derecha,
    if last_i not in pivots_idx: pivots_idx.append(int(last_i)) #y si no ha sido detectado previamente, lo añade.
...
def get_aux_data_structures(candles_df: pd.DataFrame, low_pivots_idx: list[int]) -> tuple[pd.DataFrame, pd.Series]:
    '''
    ...
    '''
    divergence_df = candles_df.iloc[low_pivots_idx].loc[:, ['low', 'rsi']].sort_index(ascending=False) #Para la determinación de la divergencia (por mínimos, el método principal).
    vol_series = candles_df.iloc[low_pivots_idx].loc[:, 'volume'] #Para el cálculo del volumen en los mínimos de la divergencia.
