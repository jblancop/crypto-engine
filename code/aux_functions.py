...
def build_candles_url(pair: str, cfg: dict[str, dict[str, Any]]) -> str:
    '''
    Construye la URL con la que obtener el DF de mercado.
    ...
    Retorno:
    + La URL correctamente formateada.  
    '''
    api_url = cfg['this']['api_url']
    candles_endpoint = cfg['this']['endpoint']['candles']['path']
    is_dynamic = cfg['this']['endpoint']['candles']['is_dynamic'] #Bitvavo tiene un punto final dinámico que incluye el par.

    if is_dynamic: candles_endpoint = f'{pair}/{candles_endpoint}'

    candles_url = f'{api_url}/{candles_endpoint}'
...
def parse_arguments() -> str:
    ...
    parser.add_argument( #El único argumento que se le pasa es la plataforma.
        '--exchange',
        required=True,
        choices=['bitunix', 'bitvavo', 'kraken', 'okx']
    )
