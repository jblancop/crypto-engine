...
class Bitunix(Exchange):
    ...
    def parse_candles(api_json, api_fields, df_fields):
        ...
        # df = df.iloc[:-1] #Se elimina la vela en formación (no hace falta si se ataca el punto final histórico)
        df = df[df_fields]
    ...
    def set_params(pair, timeframe, api_batch):
        ...
        return {
            ...
            'interval': timeframe,
            'limit': api_batch
        } #Bitunix sólo requiere "api_batch" si se ataca el punto final histórico (máximo 500 velas, no incluye la vela en formación), si no siempre devuelve 201 velas (siempre que haya suficiente histórico).