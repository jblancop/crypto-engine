...
A partir de la API de la plataforma de intercambio de criptomonedas elegida (a día de hoy, hay interfaces disponibles para Bitunix, Bitvavo, Kraken y OKX), *Crypto Engine* genera un informe de divergencias alcistas para todos los pares de mercado activos en diferentes marcos temporales (de quinceminutal a diario), que facilita al usuario la toma de decisiones de entrada.
...
└── README.md
```
En `config/config.yml` se especifican diferentes constantes de uso extendido en el proceso ETL, a las que se accede a través de la función `load_config()` de `aux_functions.py`.
